...
class Scanner:
    ...
    def ScanTokens(self):  # -> list[TokenType]:
                    ...
                    case 1:  # Si el estado actual es 1:
                        if char == "=":
                            self.tokens.append(Token(TokenType.LESS_EQUAL, "<=", None,
                                                     self.__linea))  # Añadimos un nuevo objeto Token a la lista de tokens.
                        ...
                        if char.isdigit() or char.isalpha():
                            ...
                            if current in self.palabras_reservadas:
                                ...
                                if current.lower() == "true":
                                    ...
                                    self.tokens.append(
                                        Token(self.palabras_reservadas[current], current, None, self.__linea))
                                    current = ""
                                    self.estado = 0
                            else:
        ...
        self.tokens.append(
            Token(TokenType.EOF, None, None, self.__linea - 1))  # Se termina el archivo y se agrega el token EOF
