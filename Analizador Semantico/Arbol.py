...
class Arbol:
    ...
    def solverVar(self, n:Nodo):
        ...
        if len(n.hijos) == 1:
            ...
            #print(f"this is [0] ->{n.hijos[0].value} and this is [1] -> {n.hijos[1].value}")
            if ts.simbolos.existeIdentificador(n.hijos[0].value.lexeme):
                print(f"Error: La variable {n.hijos[0].value.lexeme} ya existe")
                return
            else:
    ...
    def solverAsig(self,n:Nodo):
        #print(f"sien 0-->{n.hijos[0].value}  1-->{self.poshelp.esOperador(n.hijos[1].value.type)}")
        if ts.simbolos.existeIdentificador(n.hijos[0].value.lexeme):
            #print(f"sienx2 --> {self.poshelp.esOperador(n.hijos[1].value.type)}")
